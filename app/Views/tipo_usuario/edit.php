      ...
      <div class="row">
        <div class="col-12">
          <div class="card">
            ...
            <div class="card-body">
              <form id="editForm" action="<?= base_url('tipo_usuario/update/'.$tipo_usuario['TIPO_USUARIO_ID']) ?>" method="post">
                <div class="form-group">
                  <label for="TIPO_USUARIO_NOMBRE">Nombre</label>
                  <input type="text" name="TIPO_USUARIO_NOMBRE" class="form-control" value="<?= $tipo_usuario['TIPO_USUARIO_NOMBRE'] ?>" required>
                </div>
                <button type="button" class="btn btn-primary mt-3" onclick="confirmSave()">Guardar</button>
                <a href="<?= base_url('tipo_usuario') ?>" class="btn btn-secondary mt-3">Cancelar</a>
              </form>
            </div>
          </div>
        </div>
